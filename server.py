...
# 3. Client decrypts message, takes a SHA512 hash of it, encrypts it using server's public key, sends it to server with the IP it heard from the server prepended with a colon.
...
def get_challenge(ip):
    ...
    if ip in challenges:
        ...
        with open('/tmp/deadman_challenges.json','w') as o:
            json.dump(challenges, o)
        return base64.b64decode(bytes(ch, 'utf-8'))
    else:
        data = os.urandom(512)
...
def challenge_respond():
    ...
    try:
        ...
        ch = hashlib.sha512(challenge).digest()
        ...
        # return traceback.format_exc(), 401  # DO NOT USE IN PRODUCTION!!!
