...
def check_in(comment=None, prevent_eviction=False):
    resp = requests.post(URL('/api/checkin'), data=compose_message({'comment':comment, 'prevent_eviction':prevent_eviction}))
    print(resp, resp.text)

def add_my_key(name='Key for '+socket.gethostname()):
    resp = requests.post(URL('/api/key'), data=compose_message({'name':name, 'pubkey':str(gpg.export_keys(MY_KEY))}, use_master=True))
    print(resp, resp.text)
...
def distrust_some_key(fprint):
    resp = requests.delete(URL('/api/key/'+fprint), data=compose_message({'random':str(uuid.uuid4())}, use_master=True))
    print(resp, resp.text)
